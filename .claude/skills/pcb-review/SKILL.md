...
1. **Run the deterministic checks first:**
   ```bash
   python3 ~/pcb-review/checks/netlist_checks.py <netlist> --json
   ```
...
2. **Read every rule file:** all of `~/pcb-review/rules/*.md`. The user adds rules over time — never assume you know the list; glob and read them all fresh each run.
...
4. **Determine version and revision.** Two distinct identifiers:
   ...
   - **Revision** (`0, 1, 2, …`): the only identifier that advances review-to-review. Maintained by this pipeline. Look in the data repo (`~/Origin-Hardware-Architecture`) under `pcb-reviews/<board>/netlists/` for the highest existing `<version>-rev-N.net`. This review's revision = N+1; resets to 0 when the version bumps (i.e. after a production run).
   - **Rev-to-rev changelog**: if a previous revision's netlist exists for this version, run
     `python3 ~/pcb-review/checks/netlist_checks.py <netlist> --diff <prev-rev.net> --json`
...
7. **Publish to the app (ask once per review).** The Origin Hardware Architecture app has a "PCB Reviews" tab that displays reports from the `pcb-reviews/` folder of the data repo (`Kathan-Patel-07/Origin-Hardware-Architecture`, cloned at `~/Origin-Hardware-Architecture`). If the user wants the review visible there, commit on a feature branch + PR (never directly to a version branch):
